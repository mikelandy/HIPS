...
enum trace_arg {
    TRACE_SET_LEFT, TRACE_SET_RIGHT, TRACE_DISPLAY
    };

enum graph_alignment {
    GRAPH_VERTICAL, GRAPH_HORIZONTAL
    };
...
typedef struct trace_information {
    ...
    Windowinfo winfo;
    int leftx, rightx;
    enum graph_alignment orientation;
    int maxv;
    struct cross left, right;
    enum button_state current;
} TraceInfo;
...
#define XORP(x, y) put_pix(&info->winfo, x, y, 0xFF ^ \
    ...
    argument = va_arg(index, enum trace_arg);
