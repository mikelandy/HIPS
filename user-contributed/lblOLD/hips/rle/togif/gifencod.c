...
GIFEncode( FName, GWidth, GHeight, GInterlace, Background, 
	   BitsPerPixel, Red, Green, Blue, GetPixel, map_anyway )
...
ifunptr GetPixel;
{
	...
	/*
	 * OR in the resolution, was bug : B |= (Resolution - 1) << 5;
	 */
	B |= (Resolution - 1 << 3) & 0x70;
	...
	*/
     	if (Red) for( i=0; i<ColorMapSize; i++ ) {
            fputc( Red[i]>>8, fp );
            fputc( Green[i]>>8, fp );
            fputc( Blue[i]>>8, fp );
        } else if (map_anyway) for(i=0; i<ColorMapSize; i++)	{
		fputc( i, fp );	fputc( i, fp );	fputc( i, fp );
	}
