...
rtp_request_port(flow_id, port, flags)  int flow_id; int port; int flags;
{
...
rtp_release_port(fd)  register int	fd;
{
...
rtp_write_vec(s, data, datalen)  int s; VType* data; int datalen;
{
...
rtp_read_vec(s, data, tolerance)  int	s; char	*data; int tolerance;
{
static char*	lastbuf;
...
rtp_open(host, port, carrier, mode, buf_size)  char *host; char *port; int carrier; int mode; int buf_size;
{
...
rtp_setoption(s, option, op1, op2)  int s; int option; register int op1; int op2;
{
if (s < 0 || s >= MaxRTPchannel)	return	-1;
{
rtp_RxTx_ctrl*	rrp = rtp_resource[s];
...
rtp_RxTx_ctrl	*
get_rtp_control(fd)  int fd;
{
	if (fd < 0 || fd >= MaxRTPchannel)	return	NULL;
	return	rtp_resource[fd];
}

rtp_receive_cmd(s, rmp)  int s; rtcp_movie_t *rmp;
{
