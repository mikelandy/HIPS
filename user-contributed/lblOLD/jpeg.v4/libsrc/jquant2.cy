...
/* This version performs Floyd-Steinberg dithering */
{
  ...
  for (row = 0; row < num_rows; row++) {
    ...
    for (col = width; col > 0; col--) {
       ...
       */
#define CLAMP 32
      if (cur0 > CLAMP) cur0 = CLAMP;
      else if (cur0 < -CLAMP) cur0 = -CLAMP;
      if (cur1 > CLAMP) cur1 = CLAMP;
      else if (cur1 < -CLAMP) cur1 = -CLAMP;
      if (cur2 > CLAMP) cur2 = CLAMP;
      else if (cur2 < -CLAMP) cur2 = -CLAMP;
