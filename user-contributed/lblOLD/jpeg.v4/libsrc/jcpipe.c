...
void
single_cc_buf_init(compress_info_ptr cinfo)
{
    if (fullsize_data[0])
	free_sampling_buffer(cinfo, fullsize_data),
	fullsize_data[0] = NULL;
    if (sampled_data)	{
	register int	ci;
	for (ci=cinfo->num_components; ci--;)
...
single_ccontroller (compress_info_ptr cinfo)
{
  ...
  cinfo->total_passes++;
/*  (*cinfo->methods->write_scan_header) (cinfo);       /* for multi_frame
*/
...
single_eopt_ccontroller (compress_info_ptr cinfo)
{
  ...
  (*cinfo->methods->extract_term) (cinfo);
  (*cinfo->methods->downsample_term) (cinfo);
  (*cinfo->emethods->free_small) (sampled_data);	/* for multi_frame */
