...
#define JFIF_LEN 14
  ...
  while (length-- > 0)	{	/* skip any remaining data */
	if (JGETC(cinfo) == 0xFF && length > 2) {
		length--;
		if (JGETC(cinfo) == 0x7F)   {
			cinfo->img->frames = get_2bytes(cinfo);
			length -= 2;
		}
	}
  }
...
/* but it will never be 0 or FF */
{
  ...
  do {
    do {			/* skip any non-FF bytes */
      ...
      c = JGETC(cinfo);
/*
	if (nbytes > 1)	fprintf(stderr, "%X, %d(0x%x) %d(0x%x)	",
		c, ftell(cinfo->input_file), cinfo->input_file, ftell(cinfo->img->IN_FP), cinfo->img->IN_FP);
*/
    } while (c != 0xFF && c != EOF);
...
read_scan_header (decompress_info_ptr cinfo)
{
  ...
  default:
	return	prgmerr(0, "Unexpected marker 0x%02x", c);
    break;
  }
