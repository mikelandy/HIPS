...
int       ncol;
int       i_ocol;
int h_addm_b(),h_addm_s(),h_addm_i(),h_addm_f(),h_addm_c();

byte     *bmask;		/* binary mask	 */

int h_addmask(hdi, hdo)
    struct header *hdi, *hdo;
{
	...
	return h_addm_c((float *) hdi->firstpix,
			(float *) hdo->firstpix);
    default:	return(perr(HE_FMTSUBR,"h_addmask",
    			hformatname(hdi->pixel_format)));
    }
}				/* end of h_addmask (hdi, hdo)	 */

int h_addm_b(imagei, imageo)
    byte     *imagei;
    byte     *imageo;
{
    int       x, y;
