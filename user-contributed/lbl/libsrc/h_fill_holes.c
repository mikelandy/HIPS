...
void look_for_holes(),fill_holes(),line_fill(),i_to_r(),r_to_o(),draw_line();
int side_cnt(),corner_cnt();
...
int h_fill_holes(hdi, hdo)
    struct header *hdi, *hdo;
{
    if (hdi->pixel_format != PFBYTE)
    	return(perr(HE_FMTSUBR,"h_fill_holes",hformatname(hdi->pixel_format)));
    if (hdo->pixel_format != PFBYTE)
    	return(perr(HE_FMTSUBR,"h_fill_holes",hformatname(hdo->pixel_format)));
    i_to_r(hdi->firstpix);
    look_for_holes();
    r_to_o(hdo->firstpix);
    return(HIPS_OK);
...
/********************************************************************/
void line_fill(buf, x1, y1, x2, y2)	/* Bresenhams's scan conversion algorithm */
    byte    **buf;
    int       x1, y1, x2, y2;
 /*
