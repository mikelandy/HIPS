 ...
 */
{
  ...
  for (argn = 1; argn < argc; argn++) {
	...
	static int	hdone;
	if (!hdone)	{	/* HIPS output format. */
		format_init(&uimg, IMAGE_INIT_TYPE, HIPS, uimg.color_dpy=-1,
				"djpeg", "July6-5");
		requested_fmt = FMT_DEFAULT;	hdone++;
	}
#endif
...
extern  djpeg_dest_ptr	dest_mgr;
  ...
  jpeg_set_marker_processor(&dinfo, JPEG_COM, COM_handler);

  /* Now safe to enable signal catcher. */
#ifdef NEED_SIGNAL_CATCHER
  enable_signal_catcher((j_common_ptr) &dinfo);
#endif
  ...
  file_index = parse_switches(&dinfo, argc, argv, 0, FALSE);
  ...
  /* Open the input file. */
  if (file_index < argc) {
    if ((in_fp = fopen(argv[file_index], READ_BINARY)) == NULL) {
  ...
  if (outfilename != NULL) {
    if ((out_fp = fopen(outfilename, WRITE_BINARY)) == NULL) {
...
#ifdef PROGRESS_REPORT
  start_progress_monitor((j_common_ptr) &dinfo, &progress);
#endif

io_test(fileno(in_fp), usage());

#ifdef	STREAM_IMAGE
	format_init(&uimg, IMAGE_INIT_TYPE, HIPS, uimg.color_dpy=-1, "djpeg", "M1-4");
	if (!(*uimg.header_handle)(HEADER_READ, &uimg, 0, 0, 0))	{
#else
	if (!jpeg_header_handle(HEADER_READ, &uimg, 0, 0, 0))	{
#endif
	/* Write output file header; moved to jpeg_header_handle
		...
		uimg.cnvt = uimg.src;	/* pass pointer only - fast !	*/
	}
#endif

  /* Adjust default decompression parameters by re-parsing the options */
  file_index = parse_switches(&dinfo, argc, argv, 0, TRUE);
...
#endif
		jpeg_decompress(&dinfo, dest_mgr);
	}
...
#ifdef PROGRESS_REPORT
  end_progress_monitor((j_common_ptr) &dinfo);
#endif
