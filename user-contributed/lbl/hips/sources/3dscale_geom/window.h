...
/* $Header: window.h,v 2.0 88/10/10 13:46:30 ph Locked $ */

typedef struct {	/* WINDOW: A DISCRETE 2-D RECTANGLE */
    int x0, y0, z0;		/* xmin and ymin */
    int x1, y1, z1;		/* xmax and ymax (inclusive) */
} Window;

typedef struct {	/* WINDOW_BOX: A DISCRETE 2-D RECTANGLE */
    int x0, y0, z0;		/* xmin and ymin */
    int x1, y1, z1;		/* xmax and ymax (inclusive) */
    int nx, ny, nz;		/* xsize=x1-x0+1 and ysize=y1-y0+1 */
} Window_box;
