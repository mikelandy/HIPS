...
char *out_base;
void usageterm(),find_objects(),parse_args(),get_object_size(),mark_object();
int read_2d_byte_array(),num_neighbors();
...
void get_object_size(i, j)	/* recursive routine to count all pixels in an
				 * object */
    int       i, j;
{
...
void mark_object(i, j)		/* recursive routine to mark all pixels in an
				 * object */
    int       i, j;
{
    int       ii, jj;
