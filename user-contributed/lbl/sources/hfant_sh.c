/*
 * hfant_sh.c - Perform spacial transforms on short images. (should be "remap")
 *
...
typedef short  pixel;
#define MAXPIX 32767   /* max pixel value for type short */
...
int       originflag;		/* Center picture on given orig instead of
				 * center */
void parse_args(),xform_points(),alloc_arrays(),clear_raster(),getraster();
void xform_image(),putraster(),interp_row(),putpxl(),usageterm();
    ...
    char     *argv[];
{
	...
	fprintf(stderr, "hfant: pixel format must be short\n");
	exit(1);
    }
    ...
    hd.ocols = hd.cols = outglob.xmax + 1;
    hd.orows = hd.rows = outglob.ymax + 1;
 ...
 */
void xform_points(p, xscal, yscal, ang)
    point    *p;
 ...
 */
void interp_row(sizefac, ras_strt, real_outpos, inmax, outmax)
    float     sizefac;
    int       ras_strt;
    float     real_outpos;
    int       inmax, outmax;
    ...
    pixel  **ras_ptr;
{
    ...
    len = (inglob.xmax + 1) * sizeof(pixel);
...
/**************************************************/
/* Write out the image */
void putraster(ras_ptr)
    pixel  **ras_ptr;
{
    int       len, i;

/*
    len = ((outglob.xmax+1) * (outglob.ymax+1)) * sizeof(pixel);
    if (fwrite(ras_ptr[0], len,1,stdout) != 1)
	perr(HE_MSG,"error during write");
*/

    len = (outglob.xmax + 1) * sizeof(pixel);
...
void alloc_arrays()
{
    ...
    if ((in_rast = Calloc(ysize, pixel *)) == NULL)
	perror("calloc error: in_rast ");
    if ((in_rast[0] = Calloc(xsize * ysize, pixel)) == NULL)
	perror("calloc error: in_rast[] ");

    for (i = 1; i < ysize; i++)
	in_rast[i] = *in_rast + xsize * i;

    if ((out_rast = Calloc(ysize, pixel *)) == NULL)
	perror("calloc error: out_rast ");
    if ((out_rast[0] = Calloc(ysize * xsize, pixel)) == NULL)
	perror("calloc error: out_rast[] ");
