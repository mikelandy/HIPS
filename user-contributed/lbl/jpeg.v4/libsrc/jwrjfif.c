...
/* Emit a SOS marker */
{
  ...
  cinfo->img->tmp_offset = 8 + (i<<1);
#ifdef	_DEBUG_
	fprintf(stderr, "SOS size = %d\n", cinfo->img->tmp_offset);
#endif
}
...
/* Emit a JFIF-compliant APP0 marker */
{
   ...
   * FMarker + Frames		(4 bytes)
   */
  
  emit_marker(cinfo, M_APP0);
  
  emit_2bytes(cinfo, 2 + 4 + 1 + 2 + 1 + 2 + 2 + 1 + 1 + 4); /* length */
  ...
  emit_byte(cinfo, 0);
  emit_marker(cinfo, 0x7F);
  emit_2bytes(cinfo, (int) cinfo->img->frames);
}
...
register U_IMAGE *img = cinfo->img;
	...
	if (img->o_form != IFMT_STREAM)	{
		WRITE_BYTES(cinfo, dataptr, datacount);
	} else  {
		memcpy(img->dest, dataptr, datacount);
		img->dest = (char*)img->dest + datacount;
	}
	img->tmp_offset += datacount;
}
