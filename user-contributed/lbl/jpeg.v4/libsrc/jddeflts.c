...
#ifdef	STREAM_IMAGE
		1;	/* be set to JPEG_BUF_SIZE by JPEG */
#else
		JPEG_BUF_SIZE;
#endif
...
read_jpeg_data (decompress_info_ptr cinfo)
{
  ...
  cinfo->bytes_in_buffer = (int) JFREAD(cinfo->input_file,
					cinfo->next_input_byte,
	cinfo->img->tmp_offset ? cinfo->img->tmp_offset : jpeg_read_size);
...
/*	return	prgmerr(0, "Premature EOF in JPEG file");	*/
  }
...
/* NB: the external methods must already be set up. */
{
  ...
  if (standard_buffering) {
    verify_buffer_size(&cinfo->input_buffer, 1,
				JPEG_BUF_SIZE + MIN_UNGET, "jibuf");
