...
int write_headeru(hd,argc,argv)
...

{
	...
	if (update_header(hd,argc,argv) == HIPS_ERROR)
		return(HIPS_ERROR);
	return(fwrite_header(stdout,hd,"stdout"));
}

int write_headeru2(hd,hdp,argc,argv,flag)
...

{
	...
	if (flag) {
		...
		if (update_header(hdp,argc,argv) == HIPS_ERROR)
			return(HIPS_ERROR);
		return(fwrite_header(stdout,hdp,"stdout"));
	}
}

int write_headerun(hd,argc,argv)
...

{
	...
	if (update_headern(hd,argc,argv) == HIPS_ERROR)
		return(HIPS_ERROR);
	return(fwrite_header(stdout,hd,"stdout"));
}
...

{
	...
	while (xp != NULLPAR) {
		if (xp->count > 1) {
			switch (xp->format) {
			case PFASCII:
			case PFBYTE:	i = xp->count * sizeof(byte); break;
