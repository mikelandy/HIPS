...
#define	PFRLE		405	/* grey-scale run-length encoding */
#define PFRLED		500	/* binary run-length encoding */
#define PFRLEB		501	/* binary RLE encoding, line begins black */
#define PFRLEW		502	/* binary RLE encoding, line begins white */
...
#define	MOVIE_FRAMES	999999999	/* set hd.num_frame to this when
		operating on a movie with an unknown number of frames	*/
...
struct extpar {
	...
	union {
		...
		byte *v_pb;	/* PFBYTE/PFASCII, count > 1 */
...
/* The following can't be defined until after the header structure is defined */
