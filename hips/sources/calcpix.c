...
/*
 * calcpix.c - generate and execute a program which manipulates byte-
 *		formatted sequences according to user-defined instructions.
 *
 * usage:	calcpix [-d] [-o objectname] [-c [rows [cols]] [-f frames]
 ...
 * If the -c switch is specified (either to calcpix or to the executable file
 * created by calcpix), then the file "generates" a new sequence of the size
 ...
 * number of color frames (with -f) and the number of color planes (with -nc),
 * and the number of depth planes (with -nd), all of which default to 1.  The
 ...
 *
 * picin, ipix and opix are of type byte (which is unsigned char).  args is
 ...
 * for future use (under the name calcpix.local if the option -o was not
 * specified).
 *
 ...
 *
 * to load: cc -o calcpix calcpix.c -lhips -lm
 *
 * note that on execution the file "calcpixp.o" needs to be in directory
 * the HIPS library directory (an installation-dependent parameter often set
 * to /usr/local/lib).  calcpixp.o is the C-compiled version of calcpixp.c,
 * the main program which is loaded by calcpix.
 *
 * Yoav Cohen 8/20/82
...
static Flag_Format flagfmt[] = {
	...
	{"o",{LASTFLAG},1,{{PTFILENAME,"calcpix.local","executablefile"},
		LASTPARAMETER}},
...
extern char **args; \n\
\n\
void calcpixsr(f,nf,depth,ndepth,col,ncol,nr,nc,picin,picout,nexi,nexo) \n\
\n\
int f,nf,depth,ndepth,col,ncol,nr,nc,nexi,nexo; \n\
byte **picin,**picout; \n\
\n\
{ \n\
	byte *ipixpt,*opixpt,ipix,opix; \n\
	int i1,i2,i3,i4,i5; \n\
...

{
	...
	int rows,cols,frames,numcolor,numdepth,c;
	char *ccode,*ccodei;
	Listarg arguments;
