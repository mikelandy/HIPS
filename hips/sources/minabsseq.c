...
/*
 * minabsseq.c - compute the minimum absolute value of two images pixel by pixel
 *
 * usage:	minabsseq file <iseq >oseq
 *
 * to load:	cc -o minabsseq minabsseq.c -lhipsh -lhips
 *
...

{
	...
	for (f=0;f<fr;f++) {
		...
		if (use_hdo) {
			h_minabsp(&hdp,&hdfp,&hdo);
			write_image(&hdo,f);
		}
		else {
			h_minabsp(&hdp,&hdfp,&hdp);
			write_image(&hdp,f);
		}
	}
	return(0);
