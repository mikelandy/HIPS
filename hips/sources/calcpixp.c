...
/*
 * calcpixp.c - main program for object code produced by calcpix.c
 *
...

{
	...
	frames = hdp.num_frame/(numcolor*numdepth);
	write_headeru2(&hd,&hdp,argc,argv,hips_convback);
	dup_headern(&hdp,&hdo);
	alloc_image(&hdo);

	inrows = (byte **) memalloc(rows,sizeof(byte *));
	outrows = (byte **) memalloc(rows,sizeof(byte *));
		...
		for (col=0;col<numcolor;col++) {
			if (!cflag)
				fread_imagec(fp,&hd,&hdp,method,
					...
					filename);
			calcpixsr(f,frames,depth,numdepth,col,numcolor,rows,
