...
/* rotate180.c - rotate a frame by 180 degrees
 *
 * usage: rotate180 < frame > rotated_frame
 *
 * to load:	cc -o rotate180 rotate180.c -lhipsh -lhips -lm
 *
 * HIPS 2 - msl - 6/21/91
 */
...

{
	...
	dup_headern(&hdp,&hdo);
	getroi(&hdo,&roi);
	hd.frow = hdo.frow = hdo.orows - (roi.frow + roi.rows);
