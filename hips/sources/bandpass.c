...
/*
 * bandpass.c - apply a bandpass filter to a sequence
 *
 * usage:   bandpass  [-b | -r] [-d metric]
 *			[-f filterno | -p lowcut highcut [loworder [highorder]]]
 *				[-I | -B | -E] [-s | <inseq] >outseq
 *
 * bandpass calculates a bandpass filter MTF and either outputs that MTF as
 * a floating point image (with -s), or multiplies that MTF by an input
 * image sequence.  The user may specify one of a standard set of bandpass
 ...
 * parameterized by the cutoff frequencies, and for Butterworth and Exponential
 ...
 * To create a bandpass filter, a lowpass filter (characterized by highcut
 * and highord) and a highpass filter (characterized by lowcut and loword) are
 * concatenated (their filter MTF's are multiplied).
 ...
 *
 * To load: cc -o bandpass bandpass.c  -lhipsh -lhips -lm
 *
 * Yoav Cohen 5/21/82
