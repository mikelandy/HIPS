...
/*
 * maxabsseq.c - compute the maximum absolute value of two images pixel by pixel
 *
 * usage:	maxabsseq file <iseq >oseq
 *
 * to load:	cc -o maxabsseq maxabsseq.c -lhipsh -lhips
 *
 * The region-of-interest of file is combined with the region-of-interest of
 * iseq, then iseq with that result replacing its region-of-interest is output.
 ...
 *
 * Types handled directly: SHORT, INT, FLOAT, DOUBLE, INTPYR, FLOATPYR
 *
...
int types[] = {PFSHORT,PFINT,PFFLOAT,PFDOUBLE,PFINTPYR,PFFLOATPYR,LASTTYPE};
...

{
	...
	for (f=0;f<fr;f++) {
		...
		if (use_hdo) {
			h_maxabsp(&hdp,&hdfp,&hdo);
			write_image(&hdo,f);
		}
		else {
			h_maxabsp(&hdp,&hdfp,&hdp);
			write_image(&hdp,f);
		}
	}
	return(0);
