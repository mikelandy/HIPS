...
/*
 * calccomb.c - generate and execute a program which manipulates multiple byte-
 *		formatted sequences according to user-defined instructions.
 *
 * usage:	calccomb [-d] [-o objectname] [-A n arg1 ... argn]
 *			 [-s "statements" | -F filename]
 *			 [-i "init-statements" | -I initfilename]
 *				iseq0 iseq1 ... > oseq
 *
 * Calccomb takes c-statements from the argument list (with -s) or from an input
 ...
 *
 * picin[sq][i][j], picout[i][j], ipix(sq) and opix are of type byte (which is
 * unsigned char).  args is of type "char **".  All other variables are ints.
 ...
 * for future use (under the name calccomb.local if the option -o was not
 * specified).
 *
 ...
 *
 * to load: cc -o calccomb calccomb.c -lhips -lm
 *
 * note that on execution the file "calccombp.o" needs to be in
 * the HIPS library directory (an installation-dependent parameter often set
 * to /usr/local/lib).  calccombp.o is the C-compiled version of calccombp.c,
 * the main program which is loaded by calccomb.
 *
 * Michael Landy - 1/4/93
 * added depth - msl - 3/7/94
 */
...
static Flag_Format flagfmt[] = {
	...
	{"o",{LASTFLAG},1,{{PTFILENAME,"calccomb.local","executablefile"},
		LASTPARAMETER}},
...
extern byte **ipixpt; \n\
\n\
void calccombsr(nseq,f,nf,depth,ndepth,col,ncol,nr,nc,picin,picout,nexi,nexo) \n\
\n\
