...
/*
 * calccombp.c - main program for object code produced by calccomb.c
 *
...

{
	...
	ipixpt = (byte **) memalloc(numfiles,sizeof(byte *));
	...
	inrows = (byte ***) memalloc(numfiles,sizeof(byte **));
	for (file=0;file<numfiles;file++)
		inrows[file] = (byte **) memalloc(nrows,sizeof(byte *));
	outrows = (byte **) memalloc(nrows,sizeof(byte *));
	for(i=0;i<nrows;i++) {
		for (file=0;file<numfiles;file++)
			inrows[file][i] = hdp[file]->firstpix +
				i*hdp[file]->ocols;
	...
	for (f=0;f<frames;f++) {
		...
		for (col=0;col<numcolor;col++) {
			...
			calccombsr(numfiles,f,frames,depth,ndepth,col,numcolor,
