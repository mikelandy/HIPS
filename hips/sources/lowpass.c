...
/*
 * lowpass.c - apply a lowpass filter to a sequence
 *
 * usage:   lowpass  [-b | -r] [-d metric] [-f filterno | -p lowcut [loworder]]
 *				[-I | -B | -E] [-s | <inseq] >outseq
 *
 * lowpass calculates a lowpass filter MTF and either outputs that MTF as
 ...
 *
 * To load: cc -o lowpass lowpass.c  -lhipsh -lhips -lm
 *
 * Yoav Cohen 5/21/82
...
static Flag_Format flagfmt[] = {
	...
	{"p",{"f",LASTFLAG},1,{{PTBOOLEAN,"FALSE"},{PTDOUBLE,"0","lowcut"},
...

{
	...
	parseargs(argc,argv,flagfmt,&bflag,&rflag,&metric,&filterno,&pflag,
		&lowcut,&order,&Iflag,&Eflag,&Bflag,&sflag,FFONE,&filename);
	if (sflag) {
	...
	if (pflag) {
		...
		filt.loworder = orders[filterno];
	}
	filt.disttype = bflag ? FILTDIST_BOTH : (rflag ? FILTDIST_ROW :
		FILTDIST_COL);
	filt.ftype = FILTTYPE_LOWPASS;
	filt.dmetric = metric;
	fr = hdp.num_frame;
	for (f=0;f<fr;f++) {
		if (!sflag)
