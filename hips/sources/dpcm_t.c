...
/*
 * dpcm_t.c - DPCM transmission program.
 *
 * usage: dpcm_t [-f file-name] < inseq >oseq
 *
 ...
 *
 * To load: cc -o dpcm_t dpcm_t.c -lhips -lm
 *
 * Yoav Cohen, 6/15/82
 * HIPS 2 - msl - 7/23/91
 *
...
int current,row,col,out_encoder;
...

{
	...
	out_quant=quantizer(in-frame_mean - out_encoder);
	out_encoder=predict(inv_quantizer(out_quant)+out_encoder,estore);
	return(out_quant);
}
