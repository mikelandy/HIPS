 ...
 *
 * Usage:      morphdilate element < inseq > oseq
 *
 * Morphdilate applies the dilation operation to an image.  Although it
 ...
 * can replace it, the image pixels at the structuring element position,
 * and its former value in the input image).  Background input
 * image pixels which can not be replaced by any structural element foreground
 * pixel are left unchanged in the output image.
 *
 ...
 * ROI is centered over the pixel, so the user may determine the shift caused by
 * the dilation operation.  For example, if the element image ROI is the
 * following 5x5 image:
 *
 *	  1 255 255 255 255
 *	255 255 255 255 255
 ...
 * positions.  The ROI of the input image is dilated by the structuring
 ...
 *
 * To load:     cc -o morphdilate morphdilate.c -lhipsh -lhips -lm
 *
...

{
	...
	for (f=0;f<fr;f++) {
		...
		h_morphdil(&hdp,&hdfp,&hdo,centrr,centcc,gray);
		write_image(&hdo,f);
	}
	return(0);
}
