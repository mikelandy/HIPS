...
/*
 * diffseq.c - difference images pixel by pixel
 *
 * usage:	diffseq seq1 <seq2 >oseq
 *
 * to load:	cc -o diffseq diffseq.c -lhipsh -lhips
 *
 *
 * The region-of-interest of seq1 is subtracted from the region-of-interest of
 * seq 2, then seq2 with that difference replacing its ROI is output.
 ...
 *
 * Types handled directly: SHORT, INT, FLOAT, DOUBLE, COMPLEX, DBLCOM, INTPYR,
 *				FLOATPYR
 *
 * Hips 2 - msl - 7/3/91
 */
...
int types[] = {PFSHORT,PFINT,PFFLOAT,PFDOUBLE,PFCOMPLEX,PFDBLCOM,PFINTPYR,
		PFFLOATPYR,LASTTYPE};
...

{
	...
	if (hips_fulldesc) {
		...
		if (hd1p.sizedesc > 1) {
			desc_set2(&hd2p,HEP_SS,
			    "****%s: first sequence (minus), file: %s****\n",
			    Progname,filename1);
			desc_indentadd(&hd2p,hd1p.seq_desc);
			if (savesize > 1) {
			    desc_append2(&hd2p,HEP_SS,
			      "****%s: second sequence (plus), file: %s****\n",
			      Progname,filename2);
	...
	for (f=0;f<fr;f++) {
		...
		if (use_hdo) {
			h_diff(&hd2p,&hd1p,&hdo);
			write_image(&hdo,f);
		}
		else {
			h_diff(&hd2p,&hd1p,&hd2p);
			write_image(&hd2p,f);
		}
	}
	return(0);
