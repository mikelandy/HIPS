...
/* Process an SOI marker */
{
  ...
  if (cinfo->marker->saw_SOI && cinfo->img->frames == 1)
...
/* Process a SOFn marker */
{
  ...
  if (cinfo->marker->saw_SOF && cinfo->img->frames == 1)
...
#define JFIF_LEN 14
  ...
  INPUT_SYNC(cinfo);
#ifdef  HAVE_JPEG_FRAME_MARKER
  while (length-- > 0)  { /* scan frame counbt and skip any remaining data */
	register int	c;	/* for lots of remanent, maybe use app? */
	INPUT_BYTE(cinfo, c, return FALSE);
	if (c == 0xFF && length > 2)    {
		length--;
		INPUT_BYTE(cinfo, c, return FALSE);
		if (c == 0x7F)  {
			INPUT_2BYTES(cinfo, cinfo->img->frames, return FALSE);
			length -= 2;
		}
	}
  }
...
next_marker (j_decompress_ptr cinfo)
{
  ...
  for (;;) {
    ...
    while (c != 0xFF && c != EOF) {
      ...
      INPUT_SYNC(cinfo);
      INPUT_BYTE(cinfo, c, return FALSE);
	fprintf(stderr, "%X, %d(0x%x)	", c,
		ftell(cinfo->img->IN_FP) - bytes_in_buffer, cinfo->img->IN_FP);
    }
...
#ifdef	_DEBUG_
	if (c==0xda)	fprintf(stderr, "floc = %x ", ftell(cinfo->input_file));
#endif
