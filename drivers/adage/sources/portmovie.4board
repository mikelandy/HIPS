...
/* this is the 4 board version for systems with 4 DR256 memory boards */
...
#define	setport(imageno,screenrow,screencol,width,height) \
...

{
	...
	if (cuesw) {
		setport(cueptr,ir,ic,outr,movoutc);
		Ik_delay(60);
		Ik_delay(60);
		setport(blankptr,ir,ic,outr,movoutc);
		Ik_delay(60);
		Ik_delay(60);
	}
	...
	do {
		...
		for (frame=0;frame<numf;frame++) {
			...
			setport(sptr,ir,ic,outr,movoutc);
			if (ss!=0 && loop==0) {
